...
const ContactMethodItem: React.FC<ContactMethodProps> = ({ method, index }) => {
  return (
    <a
      href={method.href}
      target={method.href.startsWith('http') ? '_blank' : undefined}
      rel={method.href.startsWith('http') ? 'noopener noreferrer' : undefined}
      className="flex items-start p-6 bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 group animate-fade-in"
...
const ContactMethods: React.FC<ContactMethodsProps> = ({ 
  ...
  // Default contact methods using the config
  ...
  const contactMethods = methods || defaultMethods;
...
export { ContactMethodItem };
export type { ContactMethodsProps, ContactMethodProps, ContactMethod };
